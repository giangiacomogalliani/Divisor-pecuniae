...
export function DebtSummary({ users, balances, expenses, categories, settlements, suggestedPayerId, currency, onSettle }: DebtSummaryProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            {suggestedPayer && (
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20 p-6 shadow-[0_0_30px_-10px_rgba(99,102,241,0.3)] animate-in fade-in slide-in-from-bottom-2">
                    ...
                    <div className="flex items-center gap-5 relative z-10">
                        <div className="relative">
                            <div className="w-16 h-16 rounded-2xl bg-primary flex items-center justify-center text-3xl font-bold text-primary-foreground shadow-xl shadow-primary/30">
                                {suggestedPayer.name.charAt(0)}
                            </div>
                            <div className="absolute -bottom-2 -right-2 bg-background text-[10px] font-bold px-2 py-0.5 rounded-full border border-primary/30 text-primary uppercase tracking-wider shadow-sm">
                                Next
                            </div>
                        </div>
                        <div>
                            <p className="text-xs font-bold text-primary uppercase tracking-widest mb-1">Suggested Payer</p>
                                ...
                                <div key={index} className="flex items-center justify-between gap-3 p-4 rounded-2xl bg-white/5 border border-white/5">
                                    <div className="flex items-center gap-3 flex-1">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <div className="text-sm font-bold text-primary">{settlement.amount.toFixed(2)} {currency}</div>
                                        </div>
                                    </div>
                                    ...
                                    {onSettle && (
                                        <button
                                            ...
                                            className="ml-2 px-3 py-1.5 rounded-xl bg-primary/10 text-primary text-xs font-bold uppercase tracking-wider hover:bg-primary hover:text-primary-foreground transition-all border border-primary/20"
                                        >
                                            Settle
                                        </button>
                                    )}
